...
const ACHIEVEMENTS_DEF = [
  ...
  // Tarefas
  // (conquistas de tarefas removidas junto com o sistema de Tarefas)
  // XP
  ...
  // Nivel
  { id: "rank_c", name: "Classe C", desc: "Alcance a Classe C", icon: "🎖️", xp: 300, condition: s => getLevelFromXP(s.totalXP).level >= 26 },
  { id: "rank_b", name: "Classe B", desc: "Alcance a Classe B", icon: "🏅", xp: 500, condition: s => getLevelFromXP(s.totalXP).level >= 51 },
  { id: "rank_a", name: "Classe A", desc: "Alcance a Classe A", icon: "🏆", xp: 1000, condition: s => getLevelFromXP(s.totalXP).level >= 76 },
  { id: "rank_branca", name: "Sala Branca", desc: "Entre na Sala Branca", icon: "🤍", xp: 2000, condition: s => getLevelFromXP(s.totalXP).level >= 91 },
  // Habitos
...
// ─── DIFICULDADE DE HABITOS ──────────────────────────────────────────────────
const DIFFICULTY_PRESETS = [
  { label: "Facil", emoji: "🟢", xp: 10, difficulty: 2, color: "#16a34a" },
  { label: "Normal", emoji: "🔵", xp: 30, difficulty: 4, color: "#3b6bf5" },
  { label: "Medio", emoji: "🟡", xp: 50, difficulty: 6, color: "#d97706" },
  { label: "Dificil", emoji: "🟠", xp: 70, difficulty: 8, color: "#ea580c" },
  { label: "Muito dificil", emoji: "🔴", xp: 100, difficulty: 10, color: "#dc2626" },
];
...
function getLevelFromXP(xp) {
  ...
  let total = 0;
  while (true) {
    const needed = Math.floor(77 * Math.pow(level, 1.05));
...
export default function TitinFocusApp() {
  ...
  }, [state.totalXP, state.streak, state.habits, state.especiais, state.goals, state.maestria, state.relatoriosNoturnos]);
  ...
  // Valor em XP de um status: Feito = 100% do valor do habito, Parcial = 50%, Falhou = -50%
  const statusValue = (status, xp) => {
    if (status === "done") return xp;
    if (status === "partial") return Math.round(xp / 2);
    if (status === "failed") return -Math.round(xp / 2);
    return 0;
  };

  const setHabitStatus = (habitId, status, dateStr) => {
    ...
    setState(s => {
      ...
      const xpDelta = statusValue(status, xp) - statusValue(oldStatus, xp);
    ...
    const delta = statusValue(status, xp) - statusValue((habit.dailyStatus || {})[todayStr], xp);
    if (delta > 0) showToast(`+${delta} XP`, T.success);
    if (delta < 0) showToast(`${delta} XP`, T.danger);
  };

  // Sub-metas: bonus/penalidade INDEPENDENTE do valor do habito (nao divide, so soma/subtrai).
  // Cada sub-meta tem seu proprio XP. Feito = +xp da sub-meta, Falhou = -xp da sub-meta, neutro = 0.
  const subGoalValue = (status, xp) => status === "done" ? xp : status === "failed" ? -xp : 0;

  const setSubGoalStatus = (habitId, subGoalId, status, dateStr) => {
    const habit = (state.habits || []).find(h => h.id === habitId);
    if (!habit) return;
    const todayStr = dateStr || today;
    const subGoal = (habit.subGoals || []).find(sg => sg.id === subGoalId);
    if (!subGoal) return;
    const current = ((habit.subGoalStatus || {})[todayStr] || {})[subGoalId] || null;
    const newStatus = current === status ? null : status; // tocar de novo no mesmo botao = neutro (desfaz)
    const xpDelta = subGoalValue(newStatus, subGoal.xp) - subGoalValue(current, subGoal.xp);
    ...
    setState(s => ({
      ...
      habits: s.habits.map(h => {
        if (h.id !== habitId) return h;
        const dayStatus = { ...((h.subGoalStatus || {})[todayStr] || {}) };
        if (newStatus) dayStatus[subGoalId] = newStatus; else delete dayStatus[subGoalId];
        return { ...h, subGoalStatus: { ...(h.subGoalStatus || {}), [todayStr]: dayStatus } };
      }),
    }));
    if (xpDelta > 0) showToast(`+${xpDelta} XP`, T.success);
    if (xpDelta < 0) showToast(`${xpDelta} XP`, T.danger);
  };

  // ─── MISSOES DIARIAS (5 por dia da semana, definidas pelo usuario) ────────
  // Simples: feito ou nao feito. Feito = +xp da missao. Desfazer = -xp de volta.
  const toggleMissao = (missaoId, xp, dateStr) => {
    const todayStr = dateStr || today;
    const done = ((state.missoesStatus || {})[todayStr] || {})[missaoId];
    const xpDelta = done ? -xp : xp;
    setState(s => {
      const dayStatus = { ...((s.missoesStatus || {})[todayStr] || {}) };
      if (done) delete dayStatus[missaoId]; else dayStatus[missaoId] = true;
      return { ...s, totalXP: s.totalXP + xpDelta, missoesStatus: { ...(s.missoesStatus||{}), [todayStr]: dayStatus } };
    });
    if (xpDelta > 0) showToast(`+${xpDelta} XP`, T.success);
    if (xpDelta < 0) showToast(`${xpDelta} XP`, T.danger);
  };
  ...
  function HabitDetailModal({ habitId, onClose }) {
    const habit = (state.habits || []).find(h => h.id === habitId);
    if (!habit) return null;
    const dateStr = today;
    const dayStatus = (habit.subGoalStatus || {})[dateStr] || {};
    return (
      <div style={{ position:"fixed",inset:0,background:"#000c",zIndex:100,display:"flex",alignItems:"flex-end" }} onClick={onClose}>
        <div style={{ background:T.bgCard,borderRadius:"20px 20px 0 0",padding:20,width:"100%",maxWidth:480,margin:"0 auto",border:`1px solid ${T.border}`,maxHeight:"85vh",overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
          <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:12 }}>
            <div style={{ width:40,height:40,borderRadius:12,flexShrink:0,background:`${T.accent}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20 }}>{habit.icon||"🔁"}</div>
            <div style={{ flex:1 }}>
              <div style={{ fontWeight:700,fontSize:16,color:T.text }}>{habit.name}</div>
              {habit.startTime && <div style={{ fontSize:11,color:T.textMuted }}>{habit.startTime}{habit.endTime?` - ${habit.endTime}`:""}</div>}
            </div>
            <span style={{ ...styles.badge(T.warning) }}>+{habit.xp||20}xp base</span>
          </div>

          {habit.description && (
            <div style={{ ...styles.card,background:T.bgElevated,marginBottom:12 }}>
              <div style={{ color:T.textMuted,fontSize:10,fontWeight:700,letterSpacing:1,marginBottom:4 }}>COMO REALIZAR</div>
              <div style={{ color:T.text,fontSize:13,lineHeight:1.6,whiteSpace:"pre-wrap" }}>{habit.description}</div>
            </div>
          )}

          {(habit.subGoals||[]).length > 0 && (
            <div style={{ marginBottom:12 }}>
              <div style={{ color:T.textMuted,fontSize:10,fontWeight:700,letterSpacing:1,marginBottom:4 }}>SUB-METAS DE HOJE (bonus extra, alem do XP base)</div>
              <div style={{ color:T.textDim,fontSize:10,marginBottom:8 }}>Cada uma soma ou subtrai o proprio XP, independente do hábito.</div>
              {habit.subGoals.map(sg => {
                const st = dayStatus[sg.id];
                return (
                  <div key={sg.id} style={{ display:"flex",alignItems:"center",gap:8,padding:"9px 10px",borderRadius:10,border:`1px solid ${T.border}`,background:T.bgElevated,marginBottom:6 }}>
                    <span style={{ flex:1,fontSize:13,color:T.text }}>{sg.name}</span>
                    <span style={{ fontSize:10,color:T.textMuted,marginRight:2 }}>±{sg.xp}xp</span>
                    <button onClick={()=>setSubGoalStatus(habit.id, sg.id, "done", dateStr)} style={{ width:30,height:30,borderRadius:8,border:"none",background:st==="done"?T.success:`${T.success}18`,color:st==="done"?"#fff":T.success,fontWeight:700,fontSize:14,cursor:"pointer" }}>✓</button>
                    <button onClick={()=>setSubGoalStatus(habit.id, sg.id, "failed", dateStr)} style={{ width:30,height:30,borderRadius:8,border:"none",background:st==="failed"?T.danger:`${T.danger}18`,color:st==="failed"?"#fff":T.danger,fontWeight:700,fontSize:14,cursor:"pointer" }}>✕</button>
                  </div>
                );
              })}
            </div>
          ...
          <button onClick={onClose} style={{ ...styles.btn(T.textMuted),width:"100%",padding:12,textAlign:"center" }}>Fechar</button>
        </div>
      </div>
    );
  }
  ...
  function NewHabitModal() {
    const [form, setForm] = useState({ name: "", icon: "🔁", startTime: "", endTime: "", days: [0,1,2,3,4,5,6], xp: 30, difficulty: 4, description: "", subGoals: [] });
    const [newSubGoal, setNewSubGoal] = useState("");
    const [newSubGoalXp, setNewSubGoalXp] = useState(20);
    const toggleDay = (idx) => setForm(f => ({ ...f, days: f.days.includes(idx) ? f.days.filter(d=>d!==idx) : [...f.days, idx] }));

    const addSubGoal = () => {
      if (!newSubGoal.trim() || form.subGoals.length >= 7) return;
      setForm(f => ({ ...f, subGoals: [...f.subGoals, { id: Date.now(), name: newSubGoal.trim(), xp: Number(newSubGoalXp) || 10 }] }));
      setNewSubGoal("");
      setNewSubGoalXp(20);
    };
    const removeSubGoal = (id) => setForm(f => ({ ...f, subGoals: f.subGoals.filter(sg=>sg.id!==id) }));

    const save = () => {
      if (!form.name.trim()) return;
      setState(s => ({ ...s, habits: [...(s.habits||[]), { ...form, id: Date.now(), doneDates: [], dailyStatus: {}, subGoalStatus: {}, streak: 0 }] }));
      setModal(null);
      showToast("Habito criado!");
    };
    return (
      <div style={{ position:"fixed",inset:0,background:"#000c",zIndex:100,display:"flex",alignItems:"flex-end" }}>
        <div style={{ background:T.bgCard,borderRadius:"20px 20px 0 0",padding:20,width:"100%",maxWidth:480,margin:"0 auto",border:`1px solid ${T.border}`,maxHeight:"88vh",overflowY:"auto" }}>
          ...
          <div style={{ marginBottom:10 }}>
            <span style={styles.label}>Descricao (opcional)</span>
            <textarea value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))} placeholder='Ex: "Trazer chuteira. Ficar concentrado. Nao usar celular durante."' style={{ ...styles.input,minHeight:60,resize:"none" }} />
          </div>
          <div style={{ marginBottom:10 }}>
            <span style={styles.label}>Dificuldade (XP base do habito)</span>
            <div style={{ display:"flex",gap:4 }}>
              {DIFFICULTY_PRESETS.map(d => (
                <button key={d.xp} onClick={()=>setForm(f=>({...f,xp:d.xp,difficulty:d.difficulty}))} style={{ flex:1,padding:"8px 2px",borderRadius:8,border:form.xp===d.xp?`2px solid ${d.color}`:`1px solid ${T.border}`,background:form.xp===d.xp?`${d.color}18`:T.bgElevated,color:form.xp===d.xp?d.color:T.textMuted,fontWeight:700,fontSize:9,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2 }}>
                  <span style={{ fontSize:13 }}>{d.emoji}</span>
                  <span>{d.label}</span>
                  <span style={{ fontSize:9,opacity:0.8 }}>+{d.xp}xp</span>
                </button>
              ))}
            </div>
          </div>
          <div style={{ marginBottom:10 }}>
            <span style={styles.label}>Sub-metas (opcional, ate 7 — bonus/penalidade EXTRA, nao divide o XP base)</span>
            {form.subGoals.length > 0 && (
              <div style={{ marginBottom:8 }}>
                {form.subGoals.map(sg => (
                  <div key={sg.id} style={{ display:"flex",alignItems:"center",gap:8,padding:"7px 10px",background:T.bgElevated,borderRadius:8,marginBottom:4 }}>
                    <span style={{ flex:1,fontSize:12,color:T.text }}>{sg.name}</span>
                    <span style={{ ...styles.badge(T.warning) }}>±{sg.xp}xp</span>
                    <button onClick={()=>removeSubGoal(sg.id)} style={{ background:"none",border:"none",color:T.textMuted,cursor:"pointer",fontSize:12 }}>✕</button>
                  </div>
                ))}
              </div>
            )}
            {form.subGoals.length < 7 && (
              <div style={{ display:"flex",gap:6 }}>
                <input value={newSubGoal} onChange={e=>setNewSubGoal(e.target.value)} onKeyDown={e=>{ if(e.key==="Enter"){ e.preventDefault(); addSubGoal(); } }} placeholder='Ex: "Fazer bastante gol"' style={{ ...styles.input,flex:1,fontSize:12,padding:"8px 10px" }} />
                <input value={newSubGoalXp} onChange={e=>setNewSubGoalXp(e.target.value)} type="number" min="1" placeholder="XP" style={{ ...styles.input,width:64,fontSize:12,padding:"8px 6px",textAlign:"center" }} />
                <button onClick={addSubGoal} style={{ ...styles.btnSolid(T.success),padding:"8px 12px" }}>+</button>
              </div>
            )}
          </div>
          <div style={{ marginBottom:10 }}>
  ...
  // ─── IMPORTAR ROTINA (texto -> varios habitos de uma vez) ─────────────────
  function parseRoutineText(text) {
    return text
      ...
      .map(line => {
        ...
        return { name, icon: "🔁", startTime, endTime, days: [0,1,2,3,4,5,6], xp, difficulty: 4, id: Date.now() + Math.random(), dailyStatus: {}, doneDates: [], streak: 0 };
      })
      .filter(h => h.name);
  }
  ...
  function ImportRoutineModal() {
    ...
    const doImport = () => {
      ...
      setState(s => ({ ...s, habits: [...(s.habits||[]), ...parsed] }));
      setModal(null);
      showToast(`${parsed.length} habito(s) importado(s)!`);
    };
    return (
      <div style={{ position:"fixed",inset:0,background:"#000c",zIndex:100,display:"flex",alignItems:"flex-end" }}>
        <div style={{ background:T.bgCard,borderRadius:"20px 20px 0 0",padding:20,width:"100%",maxWidth:480,margin:"0 auto",border:`1px solid ${T.border}`,maxHeight:"85vh",overflowY:"auto" }}>
          ...
          <div style={{ color:T.textMuted,fontSize:11,marginBottom:12,lineHeight:1.5 }}>
            Um habito por linha, no formato <b>nome - horario - XPxp</b>. Cole aqui o texto que eu te mandar no chat. Os habitos importados entram valendo todos os dias da semana — depois voce pode ajustar os dias de cada um.
          </div>
          <textarea
            value={text}
          ...
          {preview.length > 0 && (
            <div style={{ marginBottom:12 }}>
              <div style={{ color:T.textMuted,fontSize:10,fontWeight:700,letterSpacing:1,marginBottom:6 }}>{preview.length} HABITO(S) ENCONTRADO(S)</div>
              {preview.map((h,i) => (
                <div key={i} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 10px",background:T.bgElevated,borderRadius:8,marginBottom:4 }}>
                  <span style={{ color:T.text,fontSize:12 }}>{h.name}</span>
                  <span style={{ color:T.textMuted,fontSize:11 }}>{h.startTime || "--:--"} · {h.xp}xp</span>
                </div>
              ))}
            </div>
          )}
  ...
  function HomeTab() {
    ...
    const missaoCompleta = false; // sistema de tarefas removido
    ...
    return (
      <div style={{ padding:16,paddingBottom:100 }}>
        ...
        {/* Estado vazio: conta nova, sem habitos */}
        {todayHabits.length === 0 && (
          <div style={{ ...styles.card,marginTop:12 }}>
            <div style={{ color:T.textMuted,fontSize:10,fontWeight:700,letterSpacing:2,marginBottom:10 }}>PRIMEIROS PASSOS</div>
            <div style={{ color:T.text,fontSize:13,fontWeight:700,marginBottom:4 }}>Sua jornada comeca aqui 🚀</div>
            <div style={{ color:T.textMuted,fontSize:12,marginBottom:14,lineHeight:1.5 }}>Cadastre seus primeiros habitos para ganhar XP, subir de nivel e ver seu progresso aqui na tela inicial.</div>
            <div style={{ display:"flex",gap:8,marginBottom:14 }}>
              <button onClick={()=>setTab("habits")} style={{ ...styles.btnSolid(T.accent),flex:1 }}>🔁 Criar habito</button>
            </div>
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
              <div style={{ display:"flex",gap:10,alignItems:"flex-start" }}>
                ...
                <div>
                  <div style={{ fontSize:12,fontWeight:700,color:T.text }}>Habitos</div>
                  <div style={{ fontSize:11,color:T.textMuted }}>Coisas que voce repete todo dia — treino, leitura, estudo. Use o "📋 Importar" pra cadastrar varios de uma vez.</div>
                </div>
              </div>
              <div style={{ display:"flex",gap:10,alignItems:"flex-start" }}>
                <span style={{ fontSize:16 }}>⚔️</span>
                <div>
                  <div style={{ fontSize:12,fontWeight:700,color:T.text }}>Nivel e XP</div>
                  <div style={{ fontSize:11,color:T.textMuted }}>Cada habito concluido da XP. Acumule XP pra subir de nivel e de classe.</div>
                </div>
              </div>
            </div>
          </div>
  ...
  function HabitsTab() {
    ...
    // Data real do dia selecionado (dentro da semana atual) — permite marcar status em qualquer dia ja passado
    const viewedDate = (() => {
      const d = new Date();
      d.setHours(0,0,0,0);
    ...
    const isFutureDay = viewedDate > todayMidnight;
    ...
    return (
      ...
      <div style={{ padding:16,paddingBottom:100 }}>
        ...
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10 }}>
          <div>
            <div style={{ fontWeight:700,fontSize:16,color:T.text }}>🔁 Habitos</div>
        ...
        {todayHabits.length > 0 && (() => {
          ...
          const partialCount = todayHabits.filter(h => (h.dailyStatus||{})[today] === "partial").length;
          const pct = Math.round(((doneCount + partialCount * 0.5) / todayHabits.length) * 100);
          return (
            <div style={{ ...styles.card,display:"flex",alignItems:"center",gap:12,padding:"10px 14px" }}>
              <div style={{ width:38,height:38,borderRadius:"50%",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",background:`conic-gradient(${T.success} ${pct}%, ${T.bgElevated} 0)` }}>
                <div style={{ width:29,height:29,borderRadius:"50%",background:T.bgCard,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,color:T.text }}>{pct}%</div>
              </div>
              <div style={{ flex:1 }}>
                <div style={{ fontWeight:700,fontSize:12,color:T.text }}>Progresso de hoje</div>
                <div style={{ fontSize:10.5,color:T.textMuted }}>{doneCount} feito{doneCount!==1?"s":""}{partialCount>0?`, ${partialCount} parcial${partialCount!==1?"is":""}`:""} de {todayHabits.length}</div>
              </div>
            </div>
          );
        })()}
        ...
        {dayHabits.length === 0 ? (
          ...
          return (
            <div key={habit.id} style={{ ...styles.card,borderLeft:`3px solid ${col}`,marginBottom:4,padding:"6px 6px" }}>
              <div style={{ display:"flex",alignItems:"center",gap:6 }}>
                <div onClick={()=>setDetailHabitId(habit.id)} style={{ display:"flex",alignItems:"center",gap:6,flex:1,minWidth:0,cursor:"pointer" }}>
                  <div style={{ width:19,height:19,borderRadius:5,flexShrink:0,background:`${col}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10 }}>{habit.icon||"🔁"}</div>
                  <div style={{ minWidth:0,flex:1 }}>
                    <div style={{ fontSize:12,fontWeight:700,color:T.text,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",display:"flex",alignItems:"center",gap:3 }}>
                      <span style={{ overflow:"hidden",textOverflow:"ellipsis" }}>{habit.name}</span>
                      {(habit.description || (habit.subGoals||[]).length>0) && <span style={{ color:T.textDim,fontSize:8,fontWeight:400,flexShrink:0 }}>ⓘ</span>}
                      <span style={{ ...styles.badge(T.warning),fontSize:8,padding:"1px 4px",flexShrink:0 }}>{habit.xp||20}xp base</span>
                    </div>
                    {habit.startTime && <div style={{ fontSize:7.5,color:T.textMuted }}>{habit.startTime}{habit.endTime?`-${habit.endTime}`:""}</div>}
                  </div>
                </div>
                {isFutureDay ? (
                  <span style={{ fontSize:8,color:T.textDim,flexShrink:0 }}>futuro</span>
                ) : (
                  <div style={{ display:"flex",gap:3,flexShrink:0 }}>
                    <button onClick={()=>setHabitStatus(habit.id,"done",viewedDateStr)} style={{ width:24,height:24,borderRadius:7,border:isDone?"none":`1.5px solid ${T.success}40`,background:isDone?T.success:`${T.success}12`,color:isDone?"#fff":T.success,fontWeight:700,fontSize:11,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",padding:0 }}>✓</button>
                    <button onClick={()=>setHabitStatus(habit.id,"partial",viewedDateStr)} style={{ width:24,height:24,borderRadius:7,border:isPartial?"none":`1.5px solid ${T.warning}40`,background:isPartial?T.warning:`${T.warning}12`,color:isPartial?"#fff":T.warning,fontWeight:700,fontSize:11,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",padding:0 }}>~</button>
                    <button onClick={()=>{ setHabitStatus(habit.id,"failed",viewedDateStr); setShowFailInput(habit.id); }} style={{ width:24,height:24,borderRadius:7,border:isFailed?"none":`1.5px solid ${T.danger}40`,background:isFailed?T.danger:`${T.danger}12`,color:isFailed?"#fff":T.danger,fontWeight:700,fontSize:11,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",padding:0 }}>✕</button>
                  </div>
                )}
                <button onClick={()=>deleteHabit(habit.id)} style={{ background:"none",border:"none",color:T.textDim,cursor:"pointer",fontSize:10,padding:"0 0 0 2px",flexShrink:0 }}>🗑</button>
              </div>
              {(showFailInput === habit.id || isFailed) && (
                <div style={{ marginTop:6,background:T.bgElevated,borderRadius:8,padding:8 }}>
                  <div style={{ fontSize:10,fontWeight:700,color:T.text,marginBottom:5 }}>💬 Por que nao rolou dessa vez?</div>
                  <input
                    placeholder='Ex: "Estava chovendo" ou "Bateu a preguica"...'
                    value={failReason[habit.id] ?? (habit.failReasons||{})[todayStr] ?? ""}
                    onChange={e=>setFailReason(r=>({...r,[habit.id]:e.target.value}))}
                    style={{ ...styles.input,fontSize:11 }}
                    onBlur={()=>{ setState(s=>({ ...s, habits: s.habits.map(h=>h.id===habit.id?{...h,failReasons:{...(h.failReasons||{}),[todayStr]:failReason[habit.id]??((habit.failReasons||{})[todayStr]||"")}}:h) })); setShowFailInput(null); }}
                  />
                </div>
              )}
            </div>
      ...
      {detailHabitId && <HabitDetailModal habitId={detailHabitId} onClose={()=>setDetailHabitId(null)} />}
    </>
    );
  }
  ...
  function MissoesTab() {
    ...
    const missoesDoDia = (state.missoesPorDia || {})[selectedDay] || [];
    const dayStatus = (state.missoesStatus || {})[today] || {};

    return (
      <div style={{ padding:16,paddingBottom:100 }}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12 }}>
          <div>
            <div style={{ fontWeight:700,fontSize:16,color:T.text }}>🎯 Missoes Diarias</div>
            <div style={{ color:T.textMuted,fontSize:11 }}>Ate 5 por dia da semana — voce que define</div>
          </div>
          <button onClick={()=>setEditing(!editing)} style={{ ...styles.btn(editing?T.accent:T.textMuted) }}>{editing?"✓ Pronto":"✏️ Configurar"}</button>
        </div>
        ...
        {editing ? (
          <MissoesEditor dayIdx={selectedDay} />
        ) : missoesDoDia.length === 0 ? (
          <div style={{ textAlign:"center",color:T.textMuted,padding:40 }}>
            <div style={{ fontSize:36,marginBottom:8 }}>🎯</div>
            <div style={{ fontWeight:600,color:T.text }}>Nenhuma missao configurada pra {WEEKDAYS[selectedDay].full}</div>
            <div style={{ fontSize:12,marginTop:6 }}>Toque em "Configurar" pra definir ate 5 missoes desse dia.</div>
          </div>
        ) : (
          missoesDoDia.map((m,i) => {
            const done = !!dayStatus[m.id];
            return (
              <button key={m.id} disabled={!isToday} onClick={()=>toggleMissao(m.id, m.xp, today)} style={{ width:"100%",display:"flex",alignItems:"center",gap:10,padding:"12px 14px",borderRadius:12,border:`1px solid ${done?T.success:T.border}`,background:done?`${T.success}12`:T.bgCard,cursor:isToday?"pointer":"default",marginBottom:8,textAlign:"left",opacity:isToday?1:0.6 }}>
                <span style={{ width:24,height:24,borderRadius:7,flexShrink:0,border:`2px solid ${done?T.success:T.textMuted}`,background:done?T.success:"transparent",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:14 }}>{done?"✓":""}</span>
                <span style={{ flex:1,fontSize:13,color:T.text,textDecoration:done?"line-through":"none",opacity:done?0.7:1 }}>{i+1}. {m.name}</span>
                <span style={{ ...styles.badge(T.warning) }}>+{m.xp}xp</span>
              </button>
            );
          })
        )}
        {!isToday && !editing && missoesDoDia.length > 0 && (
          <div style={{ textAlign:"center",fontSize:10,color:T.textDim,marginTop:4 }}>So da pra marcar feito no dia de hoje. Isso aqui e so uma previa de {WEEKDAYS[selectedDay].full}.</div>
        )}
      </div>
    );
  }

  function MissoesEditor({ dayIdx }) {
    const [list, setList] = useState((state.missoesPorDia||{})[dayIdx] || []);
    const [name, setName] = useState("");
    const [xp, setXp] = useState(30);

    const persist = (newList) => {
      setList(newList);
      setState(s => ({ ...s, missoesPorDia: { ...(s.missoesPorDia||{}), [dayIdx]: newList } }));
    };
    const add = () => {
      if (!name.trim() || list.length >= 5) return;
      persist([...list, { id: Date.now(), name: name.trim(), xp: Number(xp)||10 }]);
      setName(""); setXp(30);
    };
    const remove = (id) => persist(list.filter(m=>m.id!==id));

    return (
      <div>
        <div style={{ color:T.textMuted,fontSize:11,marginBottom:10 }}>Editando as missoes de <b style={{ color:T.text }}>{WEEKDAYS[dayIdx].full}</b> ({list.length}/5)</div>
        {list.map((m,i) => (
          <div key={m.id} style={{ display:"flex",alignItems:"center",gap:8,padding:"9px 12px",background:T.bgElevated,borderRadius:10,marginBottom:6 }}>
            <span style={{ flex:1,fontSize:13,color:T.text }}>{i+1}. {m.name}</span>
            <span style={{ ...styles.badge(T.warning) }}>+{m.xp}xp</span>
            <button onClick={()=>remove(m.id)} style={{ background:"none",border:"none",color:T.textMuted,cursor:"pointer",fontSize:13 }}>✕</button>
          </div>
        ))}
        {list.length < 5 && (
          <div style={{ display:"flex",gap:6,marginTop:8 }}>
            <input value={name} onChange={e=>setName(e.target.value)} onKeyDown={e=>{ if(e.key==="Enter"){ e.preventDefault(); add(); } }} placeholder="Nome da missao..." style={{ ...styles.input,flex:1,fontSize:12 }} />
            <input value={xp} onChange={e=>setXp(e.target.value)} type="number" min="1" placeholder="XP" style={{ ...styles.input,width:64,fontSize:12,textAlign:"center" }} />
            <button onClick={add} style={{ ...styles.btnSolid(T.success),padding:"8px 14px" }}>+</button>
          </div>
        )}
      </div>
    );
  ...
  // ─── DEVER DE CASA TAB ───────────────────────────────────────────────────
  function DeveresTab() {
    const deveres = state.deveresCasa || [];
    const [modalOpen, setModalOpen] = useState(false);
    const [form, setForm] = useState({ materia: "", livro: "", horarioAula: "", dataPassado: "", dataEntrega: "", descricao: "" });

    const save = () => {
      if (!form.materia.trim()) return;
      setState(s => ({ ...s, deveresCasa: [...(s.deveresCasa||[]), { ...form, id: Date.now(), done: false } ] }));
      setForm({ materia: "", livro: "", horarioAula: "", dataPassado: "", dataEntrega: "", descricao: "" });
      setModalOpen(false);
      showToast("Dever de casa adicionado!");
    };
    const toggleDone = (id) => setState(s => ({ ...s, deveresCasa: s.deveresCasa.map(d => d.id===id ? {...d, done:!d.done} : d) }));
    const remove = (id) => setState(s => ({ ...s, deveresCasa: s.deveresCasa.filter(d=>d.id!==id) }));

    const ordenados = [...deveres].sort((a,b) => {
    ...
    return (
      <div style={{ padding:16,paddingBottom:100 }}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14 }}>
          <div style={{ fontWeight:700,fontSize:16,color:T.text }}>📚 Dever de Casa</div>
          <button onClick={()=>setModalOpen(true)} style={{ ...styles.btnSolid(T.accent) }}>+ Novo</button>
        </div>

        {ordenados.length === 0 ? (
          <div style={{ textAlign:"center",color:T.textMuted,padding:40 }}>
            <div style={{ fontSize:36,marginBottom:8 }}>📚</div>
            <div style={{ fontWeight:600,color:T.text }}>Nenhum dever cadastrado</div>
          </div>
        ) : ordenados.map(d => {
          const atrasado = !d.done && d.dataEntrega && new Date(d.dataEntrega) < new Date(new Date().toDateString());
          return (
            <div key={d.id} style={{ ...styles.card,borderLeft:`3px solid ${d.done?T.success:atrasado?T.danger:T.accent}`,opacity:d.done?0.6:1 }}>
              <div style={{ display:"flex",alignItems:"flex-start",gap:10 }}>
                <button onClick={()=>toggleDone(d.id)} style={{ width:24,height:24,borderRadius:7,flexShrink:0,border:`2px solid ${d.done?T.success:T.textMuted}`,background:d.done?T.success:"transparent",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:14,cursor:"pointer",marginTop:2 }}>{d.done?"✓":""}</button>
                <div style={{ flex:1 }}>
                  <div style={{ fontWeight:700,fontSize:14,color:T.text,textDecoration:d.done?"line-through":"none" }}>{d.materia}</div>
                  {d.descricao && <div style={{ fontSize:12,color:T.textMuted,marginTop:2 }}>{d.descricao}</div>}
                  <div style={{ display:"flex",flexWrap:"wrap",gap:6,marginTop:8 }}>
                    {d.livro && <span style={{ ...styles.badge(T.purple) }}>📖 {d.livro}</span>}
                    {d.horarioAula && <span style={{ ...styles.badge(T.accent) }}>🕐 {d.horarioAula}</span>}
                    {d.dataPassado && <span style={{ ...styles.badge(T.textMuted) }}>Passado: {d.dataPassado}</span>}
                    {d.dataEntrega && <span style={{ ...styles.badge(atrasado?T.danger:T.warning) }}>{atrasado?"⚠️ ATRASADO — ":"Entregar: "}{d.dataEntrega}</span>}
                  </div>
                </div>
                <button onClick={()=>remove(d.id)} style={{ background:"none",border:"none",color:T.textMuted,cursor:"pointer",fontSize:13 }}>✕</button>
              </div>
            </div>
          );
        })}
        ...
        {modalOpen && (
          <div style={{ position:"fixed",inset:0,background:"#000c",zIndex:100,display:"flex",alignItems:"flex-end" }}>
            <div style={{ background:T.bgCard,borderRadius:"20px 20px 0 0",padding:20,width:"100%",maxWidth:1100,margin:"0 auto",border:`1px solid ${T.border}`,maxHeight:"88vh",overflowY:"auto" }}>
              <div style={{ fontWeight:700,fontSize:16,color:T.text,marginBottom:16 }}>Novo Dever de Casa</div>
              <input value={form.materia} onChange={e=>setForm(f=>({...f,materia:e.target.value}))} placeholder="Materia (ex: Matematica)" style={{ ...styles.input,marginBottom:8 }} />
              <input value={form.livro} onChange={e=>setForm(f=>({...f,livro:e.target.value}))} placeholder="Livro / material a usar" style={{ ...styles.input,marginBottom:8 }} />
              <input value={form.horarioAula} onChange={e=>setForm(f=>({...f,horarioAula:e.target.value}))} placeholder='Horario da aula (ex: "Seg e Qua, 08:00")' style={{ ...styles.input,marginBottom:8 }} />
              <div style={{ marginBottom:8 }}>
                <span style={styles.label}>Data que o professor passou</span>
                <input value={form.dataPassado} onChange={e=>setForm(f=>({...f,dataPassado:e.target.value}))} type="date" style={{ ...styles.input }} />
              </div>
              <div style={{ marginBottom:8 }}>
                <span style={styles.label}>Data de entrega</span>
                <input value={form.dataEntrega} onChange={e=>setForm(f=>({...f,dataEntrega:e.target.value}))} type="date" style={{ ...styles.input }} />
              </div>
              <textarea value={form.descricao} onChange={e=>setForm(f=>({...f,descricao:e.target.value}))} placeholder="Detalhes extras (paginas, exercicios, observacoes...)" style={{ ...styles.input,minHeight:70,resize:"none",marginBottom:8 }} />
              <div style={{ display:"flex",gap:8 }}>
                <button onClick={()=>setModalOpen(false)} style={{ ...styles.btn(T.textMuted),flex:1,padding:12 }}>Cancelar</button>
                <button onClick={save} style={{ ...styles.btnSolid(T.accent),flex:1 }}>Salvar</button>
              </div>
            </div>
          </div>
  ...
  function RelatorioNoturnoTab() {
    ...
    const todayHabits = (state.habits||[]).filter(h => getHabitDays(h).includes(todayIdx));
    const habitsDone = todayHabits.filter(h => (h.doneDates||[]).includes(today)).length;
    const pctProd = Math.round((habitsDone / Math.max(1, todayHabits.length)) * 100);
    ...
    const salvar = () => {
      setState(s => ({
        ...s,
        relatoriosNoturnos: [...(s.relatoriosNoturnos||[]), { ...form, data: today, habitsDone, pctProd }],
          ...
          <div style={{ ...styles.card,border:`1px solid ${T.accent}30` }}>
            ...
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8 }}>
              <div style={{ textAlign:"center",gridColumn:"1/-1" }}>
                <div style={{ fontSize:24,fontWeight:800,color:T.success }}>{habitsDone}/{todayHabits.length}</div>
  ...
  function RankingTab() {
    ...
    const habitsDoneTotal = (state.habits||[]).reduce((acc,h) => acc + (h.doneDates||[]).length, 0);
    ...
    const stats = [
      ...
      { label: "Habitos concluidos (total)", value: habitsDoneTotal, unit: "total", color: T.success, icon: "✅" },
        ...
        {viewMode === "semanas" && (
          <>
            <div style={{ display:"flex",gap:4,marginBottom:12,overflowX:"auto" }}>
              {weeks.map((_,i) => (
                <button key={i} onClick={()=>{ setSelectedWeek(i); setSelectedDayIdx(null); }} style={{ flexShrink:0,padding:"6px 12px",borderRadius:8,border:"none",background:selectedWeek===i?T.accent:T.bgElevated,color:selectedWeek===i?"#fff":T.textMuted,fontWeight:700,fontSize:10,cursor:"pointer" }}>{i===0?"Esta":i===1?"Ant.":`${i}s`}</button>
              ))}
            </div>
            <div style={{ color:T.gold,fontSize:12,fontWeight:700,marginBottom:12 }}>{cw.label}</div>
            <div style={{ ...styles.card,marginBottom:10 }}>
              <div style={{ color:T.textMuted,fontSize:10,letterSpacing:2,marginBottom:8 }}>HABITOS POR DIA (toque num dia pra ver detalhes)</div>
              <div style={{ display:"flex",alignItems:"flex-end",gap:4,height:70 }}>
                {cw.days.map((d,i) => (
                  <button key={i} onClick={()=>setSelectedDayIdx(selectedDayIdx===i?null:i)} style={{ flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:2,background:"none",border:"none",cursor:"pointer",padding:0 }}>
                    <div style={{ fontSize:9,color:T.textMuted }}>{d.done.length}/{d.dayHabits.length}</div>
                    <div style={{ width:"100%",height:50,background:selectedDayIdx===i?`${T.accent}30`:T.border,borderRadius:4,display:"flex",alignItems:"flex-end",overflow:"hidden" }}>
                    ...
                    <div style={{ fontSize:9,color:selectedDayIdx===i?T.accent:T.textMuted,fontWeight:selectedDayIdx===i?700:400 }}>{"DSTQQSS"[d.date.getDay()]}</div>
                  </button>
                ))}
              </div>
              {selectedDayIdx !== null && (() => {
                const d = cw.days[selectedDayIdx];
                return (
                  <div style={{ marginTop:12,paddingTop:12,borderTop:`1px solid ${T.border}` }}>
                    <div style={{ fontSize:11,fontWeight:700,color:T.text,marginBottom:8 }}>
                      {WEEKDAYS[d.date.getDay()].full}, {d.date.toLocaleDateString("pt-BR")}
                    </div>
                    {d.dayHabits.length === 0 ? (
                      <div style={{ fontSize:11,color:T.textMuted }}>Nenhum habito configurado pra esse dia da semana.</div>
                    ) : d.dayHabits.map(h => {
                      const feito = (h.doneDates||[]).includes(d.dateStr);
                      return (
                        <div key={h.id} style={{ display:"flex",alignItems:"center",gap:8,padding:"6px 0" }}>
                          <span style={{ fontSize:13 }}>{feito?"✅":"❌"}</span>
                          <span style={{ fontSize:12,color:T.text,flex:1 }}>{h.icon||"🔁"} {h.name}</span>
                          <span style={{ fontSize:10,color:feito?T.success:T.textDim }}>{feito?"feito":"nao feito"}</span>
                        </div>
                      );
                    })}
                  </div>
  ...
  const tabs = [
    ...
    { id: "missoes", icon: "🎯", label: "Missoes" },
    { id: "deveres", icon: "📚", label: "Deveres" },
  ...
  return (
    <div style={{ background:T.bg,minHeight:"100dvh",maxWidth:1100,margin:"0 auto",fontFamily:"'Segoe UI', system-ui, sans-serif",color:T.text,position:"relative",display:"flex",flexDirection:"column" }}>
      ...
      {/* Nav */}
      <div style={{ position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:1100,background:`${T.bgCard}f5`,borderTop:`1px solid ${T.border}`,display:"flex",justifyContent:"space-evenly",padding:"4px 4px 6px",backdropFilter:"blur(10px)",zIndex:20,overflowX:"auto",scrollbarWidth:"none" }}>
        {tabs.map(t => (
