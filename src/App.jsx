...
export default function CativoApp() {
  ...
  useEffect(() => {
    ...
    if (state.lastActiveDate !== today) {
      setState(s => {
        ...
        const wasYesterday = s.lastActiveDate === yesterday.toDateString();
        return { ...s, lastActiveDate: today, streak: wasYesterday ? s.streak + 1 : (s.lastActiveDate ? 0 : 1) };
      });
    }
  }, []);
  ...
  function HabitsTab() {
    ...
    const freqLabel = { daily: "Diário", weekdays: "Dias Úteis", weekend: "Fim de Semana", weekly: "Semanal" };
    const freqColor = { daily: "#3b82f6", weekdays: "#10b981", weekend: "#f59e0b", weekly: "#a855f7" };
      ...
      <div style={{ padding: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
          <div>
            ...
            <div style={{ color: C.muted, fontSize: 12 }}>{habits.filter(h => (h.doneDates || []).includes(today)).length}/{habits.length} concluídos hoje</div>
          </div>
          <button style={styles.btnSolid(C.accent)} onClick={() => setModal("newHabit")}>+ Novo</button>
        </div>

        {sorted.filter(h => h.startTime).length > 0 && (
          <div style={{ ...styles.card, marginBottom: 14 }}>
            <div style={{ color: C.muted, fontSize: 11, letterSpacing: 2, marginBottom: 10 }}>LINHA DO TEMPO — HOJE</div>
            <div style={{ position: "relative", height: 36, background: "#1f2937", borderRadius: 8, overflow: "hidden" }}>
              {sorted.filter(h => h.startTime && h.endTime).map(h => {
        ...
        {habits.length === 0 ? (
          ...
          const col = freqColor[habit.freq] || C.accent;
          ...
          return (
            <div key={habit.id} style={{ ...styles.card, marginBottom: 10, border: `1px solid ${doneToday ? (attr?.color || C.accent) + "60" : C.cardBorder}`, transition: "border-color 0.3s" }}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                <button onClick={() => toggleHabitToday(habit.id)} style={{ flexShrink: 0, width: 40, height: 40, borderRadius: "50%", border: `2px solid ${doneToday ? (attr?.color || C.accent) : C.muted}`, background: doneToday ? (attr?.color || C.accent) : "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, transition: "all 0.2s", marginTop: 2 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: "flex", gap: 6, marginTop: 8, flexWrap: "wrap", alignItems: "center" }}>
                    <span style={{ background: `${col}20`, color: col, borderRadius: 6, padding: "2px 8px", fontSize: 10, fontWeight: 700 }}>{freqLabel[habit.freq] || "Diário"}</span>
  ...
  function NewHabitModal() {
    const [form, setForm] = useState({ name: "", description: "", icon: "🔁", startTime: "", endTime: "", freq: "daily", attr: "discipline", xp: 20 });
    const icons = ["🔁","📖","💪","🧘","🏃","✍️","🥗","💧","🛌","🧠","⚔️","🎯","🎨","🎵","💻"];
    const dur = form.startTime && form.endTime ? (() => {
      const [sh, sm] = form.startTime.split(":").map(Number);
      const [eh, em] = form.endTime.split(":").map(Number);
    ...
    const save = () => {
      if (!form.name.trim()) return;
      setState(s => ({ ...s, habits: [...(s.habits || []), { ...form, id: Date.now(), doneDates: [], streak: 0 }] }));
    ...
    return (
      <ModalWrapper title="🔁 Novo Hábito">
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div><label style={styles.label}>Frequência</label>
              <select style={styles.input} value={form.freq} onChange={e => setForm(f => ({ ...f, freq: e.target.value }))}>
