...
export default function TitinFocusApp() {
  ...
  const setHabitStatus = (habitId, status, dateStr) => {
    ...
    const hasSubGoals = (habit.subGoals || []).length > 0;

    // Habitos com sub-metas: "Feito"/"Falhou" completam ou limpam todas as sub-metas de uma vez
    if (hasSubGoals && (status === "done" || status === "failed")) {
      const allIds = habit.subGoals.map(sg => sg.id);
      const doneList = (habit.subGoalDone || {})[todayStr] || [];
      let xpDelta = 0;
      let newDoneList;
      if (status === "done") {
        const toAdd = allIds.filter(id => !doneList.includes(id));
        xpDelta = toAdd.reduce((sum, id) => sum + (habit.subGoals.find(sg => sg.id === id)?.xp || 0), 0);
        newDoneList = allIds;
      } else {
        xpDelta = -doneList.reduce((sum, id) => sum + (habit.subGoals.find(sg => sg.id === id)?.xp || 0), 0);
        newDoneList = [];
      }
      setState(s => ({
        ...s,
        ...
        habits: s.habits.map(h => {
          ...
          return { ...h, subGoalDone: { ...(h.subGoalDone || {}), [todayStr]: newDoneList }, dailyStatus: newDailyStatus, doneDates };
        }),
      }));
      if (status === "done" && xpDelta > 0) showToast(`+${xpDelta} XP`, T.success);
      if (status === "failed" && xpDelta < 0) showToast(`${xpDelta} XP`, T.danger);
      return;
    }
  ...
  // Marca/desmarca uma sub-meta individual (usado dentro do modal de detalhe do habito)
  const toggleSubGoal = (habitId, subGoalId, dateStr) => {
    const habit = (state.habits || []).find(h => h.id === habitId);
    if (!habit) return;
    const todayStr = dateStr || today;
    ...
    if (!subGoal) return;
    const doneList = (habit.subGoalDone || {})[todayStr] || [];
    const isDone = doneList.includes(subGoalId);
    const newDoneList = isDone ? doneList.filter(id => id !== subGoalId) : [...doneList, subGoalId];
    const allDone = newDoneList.length === habit.subGoals.length;
    const noneDone = newDoneList.length === 0;
    const newStatus = allDone ? "done" : noneDone ? null : "partial";

    setState(s => ({
      ...s,
      totalXP: s.totalXP + (isDone ? -subGoal.xp : subGoal.xp),
      habits: s.habits.map(h => {
        if (h.id !== habitId) return h;
        const newDailyStatus = { ...(h.dailyStatus || {}) };
        if (newStatus) newDailyStatus[todayStr] = newStatus; else delete newDailyStatus[todayStr];
        const doneDates = allDone
          ? [...new Set([...(h.doneDates || []), todayStr])]
          : (h.doneDates || []).filter(d => d !== todayStr);
        return { ...h, subGoalDone: { ...(h.subGoalDone || {}), [todayStr]: newDoneList }, dailyStatus: newDailyStatus, doneDates };
      }),
    }));
    if (!isDone) showToast(`+${subGoal.xp} XP`, T.success);
  };
  ...
  function HabitDetailModal({ habitId, onClose }) {
    ...
    const dateStr = today;
    const doneList = (habit.subGoalDone || {})[dateStr] || [];
    return (
      <div style={{ position:"fixed",inset:0,background:"#000c",zIndex:100,display:"flex",alignItems:"flex-end" }} onClick={onClose}>
        <div style={{ background:T.bgCard,borderRadius:"20px 20px 0 0",padding:20,width:"100%",maxWidth:480,margin:"0 auto",border:`1px solid ${T.border}`,maxHeight:"85vh",overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
          <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:12 }}>
            ...
            <span style={{ ...styles.badge(T.warning) }}>+{habit.xp||20}xp</span>
          </div>
          ...
          {(habit.subGoals||[]).length > 0 && (
            <div style={{ marginBottom:12 }}>
              <div style={{ color:T.textMuted,fontSize:10,fontWeight:700,letterSpacing:1,marginBottom:8 }}>SUB-METAS DE HOJE</div>
              {habit.subGoals.map(sg => {
                const isDone = doneList.includes(sg.id);
                return (
                  <button key={sg.id} onClick={()=>toggleSubGoal(habit.id, sg.id, dateStr)} style={{ width:"100%",display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:10,border:`1px solid ${isDone?T.success:T.border}`,background:isDone?`${T.success}12`:T.bgElevated,cursor:"pointer",marginBottom:6,textAlign:"left" }}>
                    <span style={{ width:22,height:22,borderRadius:6,flexShrink:0,border:`2px solid ${isDone?T.success:T.textMuted}`,background:isDone?T.success:"transparent",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:13 }}>{isDone?"✓":""}</span>
                    <span style={{ flex:1,fontSize:13,color:T.text,textDecoration:isDone?"line-through":"none",opacity:isDone?0.7:1 }}>{sg.name}</span>
                    <span style={{ ...styles.badge(isDone?T.success:T.warning) }}>+{sg.xp}xp</span>
                  </button>
                );
              })}
            </div>
  ...
  function NewHabitModal() {
    ...
    // Recalcula o XP de cada sub-meta sempre que a lista muda ou o XP total muda.
    // Divide igualmente, arredondado, sem numero quebrado.
    const recalcSubGoals = (list, totalXp) => {
      if (list.length === 0) return list;
      const each = Math.round(totalXp / list.length);
      return list.map(sg => ({ ...sg, xp: each }));
    };

    const setXp = (xp, difficulty) => setForm(f => ({ ...f, xp, difficulty, subGoals: recalcSubGoals(f.subGoals, xp) }));

    const addSubGoal = () => {
      if (!newSubGoal.trim() || form.subGoals.length >= 7) return;
      setForm(f => {
        const list = [...f.subGoals, { id: Date.now(), name: newSubGoal.trim(), xp: 0 }];
        return { ...f, subGoals: recalcSubGoals(list, f.xp) };
      });
      setNewSubGoal("");
    };
    const removeSubGoal = (id) => setForm(f => ({ ...f, subGoals: recalcSubGoals(f.subGoals.filter(sg=>sg.id!==id), f.xp) }));

    const save = () => {
      if (!form.name.trim()) return;
      setState(s => ({ ...s, habits: [...(s.habits||[]), { ...form, id: Date.now(), doneDates: [], dailyStatus: {}, subGoalDone: {}, streak: 0 }] }));
      setModal(null);
    ...
    return (
      <div style={{ position:"fixed",inset:0,background:"#000c",zIndex:100,display:"flex",alignItems:"flex-end" }}>
        <div style={{ background:T.bgCard,borderRadius:"20px 20px 0 0",padding:20,width:"100%",maxWidth:480,margin:"0 auto",border:`1px solid ${T.border}`,maxHeight:"88vh",overflowY:"auto" }}>
          ...
          <div style={{ marginBottom:10 }}>
            <span style={styles.label}>Dificuldade</span>
            <div style={{ display:"flex",gap:4 }}>
              {DIFFICULTY_PRESETS.map(d => (
                <button key={d.xp} onClick={()=>setXp(d.xp,d.difficulty)} style={{ flex:1,padding:"8px 2px",borderRadius:8,border:form.xp===d.xp?`2px solid ${d.color}`:`1px solid ${T.border}`,background:form.xp===d.xp?`${d.color}18`:T.bgElevated,color:form.xp===d.xp?d.color:T.textMuted,fontWeight:700,fontSize:9,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2 }}>
          ...
          <div style={{ marginBottom:10 }}>
            <span style={styles.label}>Sub-metas (opcional, ate 7 — divide o XP entre elas)</span>
        ...
        {dayHabits.length === 0 ? (
          ...
          return (
            <div key={habit.id} style={{ ...styles.card,borderLeft:`3px solid ${col}`,marginBottom:8 }}>
              <div onClick={()=>setDetailHabitId(habit.id)} style={{ display:"flex",alignItems:"center",gap:10,marginBottom:10,cursor:"pointer" }}>
                ...
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:13,fontWeight:700,color:T.text }}>{habit.name}{(habit.description || (habit.subGoals||[]).length>0) && <span style={{ color:T.textMuted,fontSize:10,fontWeight:400 }}> · ver detalhes</span>}</div>
