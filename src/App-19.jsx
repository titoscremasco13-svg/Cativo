    ...
    return (
      ...
      <div style={{ padding:16,paddingBottom:100 }}>
        ...
        {dayHabits.length === 0 ? (
          ...
          return (
            <div key={habit.id} style={{ ...styles.card,borderLeft:`3px solid ${col}`,marginBottom:8,padding:"11px 14px" }}>
              <div onClick={()=>setDetailHabitId(habit.id)} style={{ display:"flex",alignItems:"center",gap:8,marginBottom:8,cursor:"pointer" }}>
                <div style={{ width:28,height:28,borderRadius:8,flexShrink:0,background:`${col}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13 }}>{habit.icon||"🔁"}</div>
                <div style={{ flex:1,minWidth:0 }}>
                  <div style={{ fontSize:12.5,fontWeight:700,color:T.text,display:"flex",alignItems:"center",gap:4 }}>
                    <span>{habit.name}</span>
                    {(habit.description || (habit.subGoals||[]).length>0) && <span style={{ color:T.textDim,fontSize:9,fontWeight:400 }}>ⓘ</span>}
                  </div>
                  {habit.startTime && <div style={{ fontSize:9.5,color:T.textMuted,marginTop:1 }}>{habit.startTime}{habit.endTime?` - ${habit.endTime}`:""}</div>}
                </div>
                <div style={{ display:"flex",gap:4,alignItems:"center" }}>
                  ...
                  <button onClick={(e)=>{ e.stopPropagation(); deleteHabit(habit.id); }} style={{ background:"none",border:"none",color:T.textMuted,cursor:"pointer",fontSize:12,padding:"0 2px" }}>✕</button>
                </div>
              </div>
              {isFutureDay ? (
                <div style={{ textAlign:"center",fontSize:10,color:T.textDim,padding:"4px 0" }}>Dia futuro — ainda nao da pra marcar</div>
              ) : (
                <div style={{ display:"flex",gap:8 }}>
                  <button onClick={()=>setHabitStatus(habit.id,"done",viewedDateStr)} style={{ flex:1,padding:"12px 0",borderRadius:12,border:isDone?"none":`1.5px solid ${T.success}40`,background:isDone?T.success:`${T.success}12`,color:isDone?"#fff":T.success,fontWeight:700,fontSize:20,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2,lineHeight:1 }}>
                    <span>✓</span>
                    <span style={{ fontSize:9,fontWeight:700 }}>Feito</span>
                  </button>
                  <button onClick={()=>setHabitStatus(habit.id,"partial",viewedDateStr)} style={{ flex:1,padding:"12px 0",borderRadius:12,border:isPartial?"none":`1.5px solid ${T.warning}40`,background:isPartial?T.warning:`${T.warning}12`,color:isPartial?"#fff":T.warning,fontWeight:700,fontSize:20,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2,lineHeight:1 }}>
                    <span>~</span>
                    <span style={{ fontSize:9,fontWeight:700 }}>Parcial</span>
                  </button>
                  <button onClick={()=>{ setHabitStatus(habit.id,"failed",viewedDateStr); setShowFailInput(habit.id); }} style={{ flex:1,padding:"12px 0",borderRadius:12,border:isFailed?"none":`1.5px solid ${T.danger}40`,background:isFailed?T.danger:`${T.danger}12`,color:isFailed?"#fff":T.danger,fontWeight:700,fontSize:20,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2,lineHeight:1 }}>
                    <span>✕</span>
                    <span style={{ fontSize:9,fontWeight:700 }}>Falhou</span>
                  </button>
                </div>
              )}
              {(showFailInput === habit.id || isFailed) && (
                <div style={{ marginTop:10,background:T.bgElevated,borderRadius:10,padding:10 }}>
                  <div style={{ fontSize:11,fontWeight:700,color:T.text,marginBottom:6 }}>💬 Por que nao rolou dessa vez?</div>
                  <input
