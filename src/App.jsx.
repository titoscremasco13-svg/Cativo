...
function getHabitDays(h) {
  if (Array.isArray(h.days) && h.days.length > 0) return h.days;
  switch (h.freq) {
...
export default function CativoApp() {
  ...
  function NewHabitModal() {
    const [form, setForm] = useState({ name: "", description: "", icon: "🔁", startTime: "", endTime: "", days: [0, 1, 2, 3, 4, 5, 6], attr: "discipline", xp: 20 });
    const icons = ["🔁","📖","💪","🧘","🏃","✍️","🥗","💧","🛌","🧠","⚔️","🎯","🎨","🎵","💻"];
    const dur = form.startTime && form.endTime ? (() => {
      const [sh, sm] = form.startTime.split(":").map(Number);
      const [eh, em] = form.endTime.split(":").map(Number);
