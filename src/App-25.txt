...
// Firebase config
const FB_CONFIG = {
  apiKey: "AIzaSyBWEMVWnYLpAOUVgqsrl1zkVjGqL_HSnUQ",
  authDomain: "titinfocus.firebaseapp.com",
  projectId: "titinfocus",
  storageBucket: "titinfocus.firebasestorage.app",
  messagingSenderId: "800297664273",
  appId: "1:800297664273:web:8c78e36c35c386424116b0",
};
...
export default function TitinFocusApp() {
    ...
    initFirebase().then(async ({ auth }) => {
      ...
      unsub = onAuthStateChanged(auth, async (u) => {
        ...
        if (u) {
          ...
          if (snap.exists()) setState(s => ({ ...s, ...snap.data() }));
        }
      });
  ...
  const handleLogin = async () => {
    ...
    } catch (e) { setAuthError("Email ou senha incorretos."); }
  };
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      const { doc, setDoc } = await import("https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js");
      await initFirebase();
      let existingData = initialState;
      try {
        const saved = localStorage.getItem("titinfocus_v2");
        if (saved) existingData = { ...initialState, ...JSON.parse(saved) };
      } catch (e) {}
      const cred = await createUserWithEmailAndPassword(auth, authEmail, authPassword);
      await setDoc(doc(db, "users", cred.user.uid), { ...existingData, username: authName, email: authEmail });
      setState(s => ({ ...s, ...existingData, username: authName }));
    } catch (e) { setAuthError("Erro ao criar conta. Verifique os dados."); }
  };
  ...
  if (authLoading) {
    return (
      <div style={{ background: "#0f1117", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "system-ui" }}>
        <div style={{ textAlign: "center", color: "#f59e0b" }}>
          <div style={{ fontSize: 40, marginBottom: 12 }}>⚔️</div>
          <div style={{ fontWeight: 700, fontSize: 18 }}>TITINFOCUS</div>
          <div style={{ color: "#6b7280", fontSize: 12, marginTop: 4 }}>Carregando...</div>
        </div>
      </div>
    );
  }
  ...
  if (!user) {
    return (
      <div style={{ background: "#0f1117", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "system-ui", padding: 20 }}>
        <div style={{ width: "100%", maxWidth: 380 }}>
          <div style={{ textAlign: "center", marginBottom: 32 }}>
            <div style={{ fontSize: 48, marginBottom: 8 }}>⚔️</div>
            <div style={{ color: "#f59e0b", fontWeight: 900, fontSize: 24, letterSpacing: 4 }}>TITINFOCUS</div>
            <div style={{ color: "#6b7280", fontSize: 12, marginTop: 4 }}>Foco - Disciplina - Resultados</div>
          </div>

          <div style={{ display: "flex", gap: 4, marginBottom: 20 }}>
            <button onClick={() => setAuthScreen("login")} style={{ flex: 1, padding: 10, borderRadius: 8, border: "none", background: authScreen === "login" ? "#f59e0b" : "#1f2937", color: authScreen === "login" ? "#000" : "#9ca3af", fontWeight: 700, cursor: "pointer" }}>Entrar</button>
            <button onClick={() => setAuthScreen("register")} style={{ flex: 1, padding: 10, borderRadius: 8, border: "none", background: authScreen === "register" ? "#f59e0b" : "#1f2937", color: authScreen === "register" ? "#000" : "#9ca3af", fontWeight: 700, cursor: "pointer" }}>Criar Conta</button>
          </div>

          {authScreen === "register" && (
            <input value={authName} onChange={e => setAuthName(e.target.value)} placeholder="Seu nome" style={{ width: "100%", background: "#1f2937", border: "1px solid #374151", borderRadius: 8, padding: "12px 14px", color: "#f1f5f9", fontSize: 14, marginBottom: 8, boxSizing: "border-box" }} />
          )}
          <input value={authEmail} onChange={e => setAuthEmail(e.target.value)} placeholder="Email" type="email" style={{ width: "100%", background: "#1f2937", border: "1px solid #374151", borderRadius: 8, padding: "12px 14px", color: "#f1f5f9", fontSize: 14, marginBottom: 8, boxSizing: "border-box" }} />
          <input value={authPassword} onChange={e => setAuthPassword(e.target.value)} placeholder="Senha" type="password" style={{ width: "100%", background: "#1f2937", border: "1px solid #374151", borderRadius: 8, padding: "12px 14px", color: "#f1f5f9", fontSize: 14, marginBottom: 8, boxSizing: "border-box" }} />

          {authError && <div style={{ color: "#ef4444", fontSize: 12, marginBottom: 8 }}>{authError}</div>}

          <button onClick={authScreen === "login" ? handleLogin : handleRegister} style={{ width: "100%", background: "#f59e0b", border: "none", borderRadius: 8, padding: 14, color: "#000", fontWeight: 700, fontSize: 15, cursor: "pointer", marginBottom: 12 }}>
            {authScreen === "login" ? "Entrar" : "Criar Conta"}
          </button>

          <div style={{ color: "#6b7280", fontSize: 11, textAlign: "center" }}>Seus dados ficam salvos na nuvem e sincronizados em todos os dispositivos.</div>
        </div>
      </div>
    );
  }
      ...
      <div style={{ background: `linear-gradient(180deg, ${rank.bg}cc, transparent)`, padding: "16px 16px 0", position: "sticky", top: 0, zIndex: 10 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingBottom: 12 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            {syncing && <span style={{ color: "#6b7280", fontSize: 10 }}>☁️</span>}
            <button onClick={handleLogout} style={{ background: "#1f2937", border: "none", borderRadius: 8, padding: "4px 8px", color: "#6b7280", fontSize: 10, cursor: "pointer" }}>Sair</button>
